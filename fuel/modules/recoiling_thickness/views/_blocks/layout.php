...
function loadfolderlist(account, accname) {
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo fuel_url('recoiling_thickness/listratethickness');?>",
        data: "coil=" + account,
        dataType: "json"
        }).done(function( msg ) {
			if(msg.length == 0) {
 ...
 function deleteItem(pd){
    ...
    if(checkstr == true){
	  ...
	  type: "POST",  
	  data: dataString,
		url	: "<?php echo fuel_url('recoiling_thickness/deleteratethic_coil');?>/?"+ dataString+"",
...
var dataString = 'minthickness='+minthickness+'&coil='+coil;
  $.ajax({
  url	: "<?php echo fuel_url('recoiling_thickness/checkthickness');?>/",
  success: function(msg){  
			alert(msg); 
			}
  });
...
function minthickness_exist()
{
	...
	var dataString = 'minthickness='+minthickness+'&coil='+coil;
	  ...
	  type: "POST",  
	  data: dataString,
	  url	: "<?php echo fuel_url('recoiling_thickness/minthickness');?>/",
	  success: function(msg){ 
	  if(msg == '1'){
...
function maxthickness_exist()
{
	...
	var dataString = 'maxthickness='+maxthickness+'&coil='+coil;
	  ...
	  type: "POST",  
	  data: dataString,
	  url	: "<?php echo fuel_url('recoiling_thickness/maxthickness');?>/",
	  success: function(msg){ 
	  if(msg == '1'){
...
function functionsave()
{
	...
	var dataString = 'coildescription='+coildescription+'&minthickness='+minthickness+'&maxthickness='+maxthickness+'&rate='+rate;
		$.ajax({  
			type: "POST",  
			url	: "<?php echo fuel_url('recoiling_thickness/saveratedetails');?>/",  
			data: dataString,
			success: function(msg){  
...
function update()
{
	...
	var dataString = 'priceid='+priceid+'&minthickness='+minthickness+'&maxthickness='+maxthickness+'&rate='+rate;
		$.ajax({  
			type: "POST",  
			url	: "<?php echo fuel_url('recoiling_thickness/updateratedetails');?>/",
data: dataString,			
			success: function(msg){  
...
var dataString = 'width='+width;
  $.ajax({  
			type: "POST",  
			url	: "<?php echo fuel_url('recoiling_thickness/tablethickness');?>/",  
			data: dataString,
			success: function(msg){  
			}  
		});
